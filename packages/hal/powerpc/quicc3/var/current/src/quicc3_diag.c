...
init_serial_channel(channel_data_t* __ch_data)
{
    ...
    cyg_uint8 lcr;
    cyg_uint16 baud_divisor =  (cyg_uint16)(((cyg_uint32)CYGHWR_HAL_POWERPC_CPU_SPEED  * 1000000) / ((cyg_uint32)(CYGNUM_HAL_VIRTUAL_VECTOR_CONSOLE_CHANNEL_BAUD) * 16));
...
static channel_data_t channels[2] = {
    { (cyg_uint8*)(CYGARC_IMM_BASE + UART1_OFFSET), 1000, CYGNUM_HAL_INTERRUPT_UART},
#if (CYGNUM_HAL_VIRTUAL_VECTOR_COMM_CHANNELS > 1)
    { (cyg_uint8*)(CYGARC_IMM_BASE + UART2_OFFSET), 1000, CYGNUM_HAL_INTERRUPT_UART}
#endif
};
...
cyg_hal_plf_serial_init(void)
{
    ...
    // Disable interrupts.
#if !(defined(CYG_HAL_STARTUP_RAM) && !defined(CYGSEM_HAL_USE_ROM_MONITOR)) /* CC 2009 temp fix, diag_printf not working in RAM application */    
