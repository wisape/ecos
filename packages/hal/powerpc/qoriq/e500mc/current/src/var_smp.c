...
// Contributors: Updated for Freescale QorIQ platform by ccoutand
// Date:         2012-07-20
// Purpose:      HAL SMP implementation for Freescale QorIQ
...
{
  volatile cyg_uint32 *lcc_bstrh =   (volatile cyg_uint32 *) \
    (CYGARC_IMM_BASE + CYGARC_REG_IMM_LCC_BSTRH);
  volatile cyg_uint32 *lcc_bstrl =   (volatile cyg_uint32 *) \
    (CYGARC_IMM_BASE + CYGARC_REG_IMM_LCC_BSTRL);
  volatile cyg_uint32 *lcc_bstar =   (volatile cyg_uint32 *) \
    (CYGARC_IMM_BASE + CYGARC_REG_IMM_LCC_BSTAR);

  //
  // The ROM startup type means booting from NOR flash. Booting from
  // the NAND flash is not supported.
  //
#ifdef CYG_HAL_STARTUP_RAM
  cyg_uint32 bstar = CYGARC_REG_IMM_LCC_BSTAR_SET_TRGT_ID(CYGARC_TRANS_DST_ID_INTL_MEM_COMPLEX1_2);
#else
  cyg_uint32 bstar = CYGARC_REG_IMM_LCC_BSTAR_SET_TRGT_ID(CYGARC_TRANS_DST_ID_eLBC) ;
#endif

 bstar |= (CYGARC_REG_IMM_LCC_BSTAR_EN | CYGARC_REG_IMM_LCC_BSTAR_4K);

  *lcc_bstrh = 0x0;
  *lcc_bstrl = ((cyg_uint32) &_cyg_hal_slave_trampoline);
  *lcc_bstar = bstar;
...
{
  volatile cyg_uint32 *brr =   (volatile cyg_uint32 *) \
    (CYGARC_IMM_BASE + CYGARC_REG_IMM_DCFG + CYGARC_REG_IMM_DCFG_BRR);

  // Start other cores. Not all cores, only the number that
  // was specified for this SMP HAL
  *brr |= ( 1 << cpu );
