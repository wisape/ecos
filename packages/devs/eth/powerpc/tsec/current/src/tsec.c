...
//#####DESCRIPTIONBEGIN####
//
// Author(s):    Edgar Grimberg, ccoutand
// Contributors: Sachin Sushil Chaddha
// Date:         2009-11-01
// Purpose:      
...
#include CYGDAT_DEVS_TSEC_ETH_PLF_H
#include CYGDAT_DEVS_TSEC_ETH_PLF_INL
       ...
       HAL_SavedRegisters *regs)
{
  ...
  if( vector == qi->rx_irq_vector )
     qi->rx_mask = true;
  else if( vector == qi->tx_irq_vector )
     qi->tx_mask = true;
  else if( vector == qi->err_irq_vector )
     qi->err_mask = true;
...
#ifdef CYGPKG_KERNEL_SMP_SUPPORT  
    cyg_drv_spinlock_spin( &qi->tx_spin_lock );
    cyg_drv_spinlock_spin( &qi->rx_spin_lock );
#endif
...
static void tsec_eth_int(struct eth_drv_sc *sc)
{
  ...
  while ((event = qi->tsec->ievent) != 0)
  {
    ...
    {
      tsec_printf("TSEC%d - Receive packet\n", qi->tsec_if);
      tsec_eth_RxEvent(sc);
    }
    if (( event & IEVENT_TXF ) != 0)
    {
      tsec_printf("TSEC%d - Transmit packet\n", qi->tsec_if);
      tsec_eth_TxEvent(sc);
    }
  }
 ...
 */
static 
void tsec_disp_stats(struct eth_drv_sc *sc , char flag )
{
  struct tsec_eth_info *qi = (struct tsec_eth_info *) sc->driver_private;
#ifdef CYGPKG_KERNEL_SMP_SUPPORT
  int i;
...
#ifdef DEBUG_IRQ
    diag_printf("RX mask: %s, TX mask: %s, Err mask: %s\n", \
      ((cyg_hal_interrupt_status( qi->rx_irq_vector ) == 1) ? "yes" : "no" ), \
      ((cyg_hal_interrupt_status( qi->tx_irq_vector ) == 1) ? "yes" : "no" ), \
      ((cyg_hal_interrupt_status( qi->err_irq_vector ) == 1) ? "yes" : "no" ));
#endif
...
static
void tsec_disp_bd_stats(struct eth_drv_sc *sc , char flag )
{
  int i, current_bd = -1;
  struct tsec_eth_info *qi = (struct tsec_eth_info *) sc->driver_private;
  volatile struct tsec_bd *rxbd;

  rxbd = qi->rbase;

  if( flag & TSEC_STAT_ALL_BDS )
  {
    diag_printf("Idx - Empty - Wrap -  Len\n");
  }
  ...
  for (i = 0; i < CYGNUM_DEVS_ETH_POWERPC_TSEC_RxNUM; i++)
  {
    if( flag & TSEC_STAT_ALL_BDS )  
    {
      diag_printf("%3d - %5d - %4d - %4d\n", 
         i, \
         ((rxbd->ctrl & FEC_BD_Rx_Empty) ? 1 : 0), \
         ((rxbd->ctrl & FEC_BD_Rx_Wrap) ? 1 : 0), \
         rxbd->length);
      }
      if(rxbd == qi->rnext)
         current_bd = i;
...
externC
void disp_eth_stats(int eth, char flag)
{
#if defined(CYGPKG_DEVS_ETH_MPC8572DS_ETH0)
   if(eth == 0)
   {
     tsec_disp_stats( &tsec_eth0_sc , flag);
     tsec_disp_bd_stats( &tsec_eth0_sc , flag); 
   }
#endif   
#if defined(CYGPKG_DEVS_ETH_MPC8572DS_ETH1)
   if(eth == 1)
   {
     tsec_disp_stats( &tsec_eth1_sc , flag);
     tsec_disp_bd_stats( &tsec_eth1_sc , flag); 
   }
#endif   
#if defined(CYGPKG_DEVS_ETH_MPC8572DS_ETH2)
   if(eth == 2)
   {
     tsec_disp_stats( &tsec_eth2_sc , flag );
     tsec_disp_bd_stats( &tsec_eth2_sc , flag ); 
   }
#endif  
#if defined(CYGPKG_DEVS_ETH_MPC8572DS_ETH3)
   if(eth == 3)
   {
     tsec_disp_stats( &tsec_eth3_sc , flag );
     tsec_disp_bd_stats( &tsec_eth3_sc , flag ); 
   }
#endif  
}
