...
struct tsec_eth_info {
    ...
    int                            rxmax;             // Debug RX counter
#ifdef CYGPKG_KERNEL_SMP_SUPPORT    
    char                           rx_irq_in_progress;
#endif
    ...
    volatile struct tsec_bd        *init_rxring;       // Initial base pointer of RX ring
    volatile struct tsec_bd        *init_txring;       // Initial base pointer of TX ring
...
#define TSEC_ERROR_BABR(qi) \
   diag_printf("TSEC%d - Babbling receive error.\n", qi->tsec_if)
#define TSEC_ERROR_BSY(qi) \
   diag_printf("TSEC%d - Busy condition interrupt.\n", qi->tsec_if)
#define TSEC_ERROR_EBERR(qi) \
   diag_printf("TSEC%d - Internal bus error.\n", qi->tsec_if)
#define TSEC_ERROR_MSRO(qi) \
      diag_printf("TSEC%d - MIB counter overflow.\n", qi->tsec_if)
#define TSEC_ERROR_BABT(qi) \
      diag_printf("TSEC%d - Babbling transmit error.\n", qi->tsec_if)
#define TSEC_ERROR_TXE(qi) \
      diag_printf("TSEC%d - Transmit error.\n", qi->tsec_if)
#define TSEC_ERROR_LC(qi) \
      diag_printf("TSEC%d - Late collision.\n", qi->tsec_if)
#define TSEC_ERROR_CRL(qi) \
      diag_printf("TSEC%d - Collision retry limit.\n", qi->tsec_if)
#define TSEC_ERROR_XFUN(qi) \
      diag_printf("TSEC%d - Transmit FIFO underrun.\n", qi->tsec_if)
#define TSEC_ERROR_DPE(qi) \
      diag_printf("TSEC%d - Internal data parity error.\n", qi->tsec_if)

#define TSEC_STAT_ALL_BDS        0x01
#define TSEC_STAT_CURRENT_BD 0x02
#define TSEC_STAT_RMON            0x04
#define TSEC_STAT_SMP             0x08
