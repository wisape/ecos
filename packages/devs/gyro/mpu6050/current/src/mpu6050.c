...
{
    int ret = 0;
    const cyg_i2c_device* i2c_dev = &i2c_mpu6050;
    ...
    cyg_i2c_transaction_begin(i2c_dev);
    if(!cyg_i2c_transaction_tx(i2c_dev, true, &buffer[0], size + 1, true)) {
	ret = -1;
    }
    cyg_i2c_transaction_end(i2c_dev);
			...
			cyg_uint8  size, cyg_uint8 *pdata)
{
    ...
    cyg_i2c_transaction_begin(i2c_dev);
    if(!cyg_i2c_transaction_tx(i2c_dev, true, &buffer[0], 1, false)) {
        ret = -1;
    } else if(!cyg_i2c_transaction_rx(i2c_dev, true, &input[0], size, true, true)) {
	ret = -1;
    }
    cyg_i2c_transaction_end(i2c_dev);
...
#else
	mpu_read_fifo(data->gyro, data->accel,
			&data->sensor_timestamp, &sensors, &more);
#endif
