//=============================================================================
//
//      flash_mini_stm32_test.c
//
//      SPI flash driver tests for Mini STM32 evaluation board.
//
//=============================================================================
...
// Date:        2011-04-25
// Purpose:     Silicon Storage Technology SST25xx SPI flash driver stress tests.
//
//####DESCRIPTIONEND####
//
//=============================================================================
...
cyg_flash_info_t sst25vf0161b_flash_info;
...
#endif
    diag_printf("RAM: %p-%p, ", (void*)(CYGMEM_REGION_sram), (void*)(CYGMEM_REGION_sram + CYGMEM_REGION_sram_SIZE - 1));
  ...
  (void)
{
    ...
    // Unlock device
    status = cyg_flash_unlock (sst25vf0161b_flash_info.start, 1, &err_addr);
    ...
    for (i = 0; i < sst25vf0161b_flash_info.block_info->blocks; i++) {
        base_addr =  sst25vf0161b_flash_info.start + (i * sst25vf0161b_flash_info.block_info->block_size);
        // Erase block.
        ...
        for (j = 0; j < sst25vf0161b_flash_info.block_info->block_size ; j += BUF_SIZE) {
  ...
  (void)
{
    ...
    base_addr =  sst25vf0161b_flash_info.start;
    for (i = 0; i < (sst25vf0161b_flash_info.end - sst25vf0161b_flash_info.start) / 4;) {
        for (j = 0; j < BUF_SIZE;) {
  ...
  (void)
{
    ...
    base_addr = sst25vf0161b_flash_info.start + sst25vf0161b_flash_info.block_info->block_size;
    for (i = 1; i <= 256; i++) {
        ...
        base_addr += sst25vf0161b_flash_info.block_info->block_size;
        if (base_addr >= sst25vf0161b_flash_info.end)
            base_addr = sst25vf0161b_flash_info.start + sst25vf0161b_flash_info.block_info->block_size;
    }
    return errors;
}
...
//-----------------------------------------------------------------------------
// Run all SST25xx SPI flash tests
    ...
    (void)
{
    ...
    cyg_flash_get_info_addr(0x80000000, &sst25vf0161b_flash_info);

    diag_printf ("SST25xx start: %8x\n", sst25vf0161b_flash_info.start);
    diag_printf ("SST25xx end:   %8x\n", sst25vf0161b_flash_info.end);

    // Check that the device is initialized.
    if (sst25vf0161b_flash_info.start == sst25vf0161b_flash_info.end) {
        diag_printf ("SST25xx device not initialized.\n");
        errors ++;
        goto out;
    }
