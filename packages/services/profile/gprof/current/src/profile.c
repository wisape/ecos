...
# define PROFILE_TFTP_CPU_MAX 4
#endif

#ifdef CYGSEM_PROFILE_PER_CORE
# include <cyg/hal/hal_smp.h>
# define PROFILE_CPU_MAX CYGPKG_HAL_SMP_CPU_MAX
#else
...
static cyg_uint16*      profile_hist_data[PROFILE_CPU_MAX];
...
static struct profile_arc*  profile_arc_records[PROFILE_CPU_MAX];

// The next free slot in the arc_records table.
static int                  profile_arc_next[PROFILE_CPU_MAX]    = { 1 };
...
// of places.
static int*         profile_arc_hashtable[PROFILE_CPU_MAX];
...
static int          profile_arc_overflow[PROFILE_CPU_MAX];
#endif
...
profile_reset(void)
{
    ...
    for(nloop = 0; nloop < PROFILE_CPU_MAX; nloop++) {
       memset(profile_hist_data[nloop], 0, profile_hist_hdr.hist_size * sizeof(cyg_uint16));
    }
#ifdef CYGPKG_PROFILE_CALLGRAPH
    ...
    for(nloop = 0; nloop < PROFILE_CPU_MAX; nloop++) {
       memset(profile_arc_hashtable[nloop], 0, profile_arc_hash_count * sizeof(int));
       profile_arc_next[nloop]     = 1;
       profile_arc_overflow[nloop] = 0;
   }
#endif
}
...
__profile_hit(CYG_ADDRWORD pc)
{
    ...
    if ((pc >= (CYG_ADDRWORD)profile_hist_hdr.low_pc) && (pc <= (CYG_ADDRWORD)profile_hist_hdr.high_pc)) {
        ...
        if (profile_hist_data[me][bucket] < (unsigned short)0xFFFF) {
            profile_hist_data[me][bucket]++;
        }
    }
}
...
__profile_mcount(CYG_ADDRWORD caller_pc, CYG_ADDRWORD callee_pc)
{
    ...
    if (0 == profile_arc_hashtable[me][hash_index]) {
        if (profile_arc_next[me] == profile_arc_records_count) {
            profile_arc_overflow[me] = 1;
        } else {
            profile_arc_hashtable[me][hash_index]   = profile_arc_next[me];
            current = &(profile_arc_records[me][profile_arc_next[me]]);
            profile_arc_next[me]++;
            current->next   = 0;
        ...
        current = &(profile_arc_records[me][profile_arc_hashtable[me][hash_index]]);
        while (1) {
            if ((current->record.from_pc == (void*) caller_pc) && (current->record.self_pc == (void*) callee_pc)) {
                ...
                if (profile_arc_next[me] == profile_arc_records_count) {
                    ...
                    current->next   = profile_arc_next[me];
                    current         = &(profile_arc_records[me][profile_arc_next[me]]);
                    profile_arc_next[me]++;
                    current->next   = 0;
                    current->record.from_pc = (void*) caller_pc;
                ...
                current = &(profile_arc_records[me][current->next]);
            }
        }
    }
}
...
static int              profile_tftp_core_id        = -1;

static int
profile_tftp_open(const char *filename, int flags)
{
    char *gmon_name[PROFILE_TFTP_CPU_MAX]=
      {"gmon.out",   // CPU core 0
       "gmon1.out",  // CPU core 1
       "gmon2.out",  // CPU core 2
       "gmon3.out"   // CPU core 3
    };
    cyg_uint8 nloop      = 0;
    profile_tftp_core_id = -1;
    ...
    for(nloop = 0 ; nloop < PROFILE_TFTP_CPU_MAX; nloop++) {
        // Only gmon.out can be retrieved using this tftp daemon
        if (0 == strcmp(filename, gmon_name[nloop])) {
            profile_tftp_core_id = nloop;
            break;
        }
    }
    if( profile_tftp_core_id == -1 )
        return -1;
...
#ifdef CYGPKG_PROFILE_CALLGRAPH
    if (profile_arc_overflow[profile_tftp_core_id]) {
...
profile_tftp_read_next(void)
{
    switch (profile_tftp_next_index) {
      ...
      case 3 :      // The histogram data
        profile_tftp_current_block  = (unsigned char*) profile_hist_data[profile_tftp_core_id];
      ...
      default :     // One of the arc records. These start at array offset 1.
        {
            ...
            if (arc_index >= profile_arc_next[profile_tftp_core_id]) {
                profile_tftp_current_block  = (unsigned char*) 0;
                profile_tftp_current_len    = 0;
            } else {
                // gmon.out should contain a 1 byte tag followed by each
                // arc record.
                profile_tftp_current_block  = (unsigned char*) &(profile_arc_records[profile_tftp_core_id][arc_index].tags[3]);
...
profile_off(void)
{
    ...
    profile_reset();
    for(nloop = 0; nloop < PROFILE_CPU_MAX; nloop++) {
       if (profile_hist_data[nloop]) {
          free(profile_hist_data[nloop]);
          profile_hist_data[nloop] = NULL;
       }
    }
#ifdef CYGPKG_PROFILE_CALLGRAPH
    for(nloop = 0; nloop < PROFILE_CPU_MAX; nloop++) {
       if (profile_arc_hashtable[nloop]) {
          free(profile_arc_hashtable[nloop]);
          profile_arc_hashtable[nloop]=NULL;
       }
       if (profile_arc_records[nloop]) {
          free(profile_arc_records[nloop]);
          profile_arc_records[nloop]=NULL;
       }
    }
#endif
}
...
profile_on(void *_start, void *_end, int _bucket_size, int resolution)
{
    ...
    for(nloop = 0 ; nloop < PROFILE_CPU_MAX ; nloop++) {
       profile_hist_data[nloop] = (cyg_uint16*) malloc(profile_hist_hdr.hist_size * sizeof(cyg_uint16));
       if ((cyg_uint16*)0 == profile_hist_data[nloop]) {
    ...
    // programs (including testcases).
    {
        ...
        for(nloop = 0 ; nloop < PROFILE_CPU_MAX ; nloop++) {
           profile_arc_hashtable[nloop] = (int*) malloc(profile_arc_hash_count * sizeof(int));
           if ((int*)0 == profile_arc_hashtable[nloop]) {
               diag_printf("profile_on(): cannot allocate call graph hash table\n  call graph profiling disabled\n");
           } else {
               memset(profile_arc_hashtable[nloop], 0, profile_arc_hash_count * sizeof(int));
               profile_arc_records[nloop] = (struct profile_arc*) malloc(profile_arc_records_count * sizeof(struct profile_arc));
               if ((struct profile_arc*)0 == profile_arc_records[nloop]) {
                   diag_printf("profile_on(): cannot allocate call graph arc table\n  call graph profiling disabled\n");
                   free(profile_arc_hashtable[nloop]);
                   profile_arc_hashtable[nloop] = (int*) 0;
               } else {
                   memset(profile_arc_records[nloop], 0, profile_arc_records_count * sizeof(struct profile_arc));
                   for (i = 0; i < profile_arc_records_count; i++) {
                       profile_arc_records[nloop][i].tags[3] = GMON_TAG_CG_ARC;
                   }
                   profile_arc_next[nloop] = 1;    // slot 0 cannot be used because 0 marks an unused hash slot.
               }
           }
        }
    }
#else
    for(nloop = 0 ; nloop < PROFILE_CPU_MAX ; nloop++) {
       profile_arc_records[nloop]     = (struct profile_arc*) 0;
    }
#endif

    diag_printf("Profile from %p..%p in %d buckets of size %d. Profiling %d core(s)\n",
                profile_hist_hdr.low_pc, profile_hist_hdr.high_pc,
                profile_hist_hdr.hist_size, bucket_size, PROFILE_CPU_MAX);
