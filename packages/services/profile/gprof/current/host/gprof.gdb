...
define gprof_fetch
  if 0 == profile_hist_data[0]
    ...
    if 0 != profile_reset_pending
      ...
      append binary memory gmon.out profile_hist_data[0] (profile_hist_data[0] + profile_hist_hdr.hist_size)
      if 0 != profile_arc_records[0]
        if 0 != profile_arc_overflow[0]
        ...
        set $profile_i = 1
        set $profile_count = profile_arc_next[0]
        while $profile_i != $profile_count
          append binary memory gmon.out &(profile_arc_records[0][$profile_i].tags[3]) &(profile_arc_records[0][$profile_i + 1])
